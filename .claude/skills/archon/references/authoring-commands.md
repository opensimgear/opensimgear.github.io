...
Commands are plain Markdown files containing AI prompt templates. They are the atomic unit of AI instruction — each
command file defines what a single AI agent does in one step of a workflow.
...
The frontmatter is metadata for discovery. The entire file content (including frontmatter) becomes the prompt sent to
the AI.
...
- **Vague instructions** — "Fix the code" is too vague. Be specific about what to investigate, which tools to use, what
  output to produce.
- **No artifact output** — If a command produces no artifacts, downstream steps have nothing to work from.
- **Assuming prior context** — When `context: fresh` is set on the calling node, the AI starts fresh. It must read
  artifacts explicitly.
- **Giant monolithic commands** — Split complex work into focused phases. Each command should have one clear
  responsibility.
...
Analyze the codebase and help the user with their request. Use Read, Grep, Glob, and Bash tools to explore the code.
