...
describe('human rig export script', () => {
  it('derives the non-deforming eye center bone from the head bone vector', () => {
    ...
    expect(eyeCenterHelper).not.toContain('normalized()');
    expect(eyeCenterHelper).not.toContain('normalize()');
    ...
    expect(debugOrder).toContain('eyeCenter');
  });
});
